...
class BilinearSpikeSlabRBM(Model, Block):
    ...
    def load_params(self, model):
        ...
        self.cg.set_value(model.cg.get_value())
        self.ch.set_value(model.ch.get_value())
        self.gamma_s.set_value(model.gamma_s.get_value())
        self.gamma_t.set_value(model.gamma_t.get_value())
        self.linproj_v_std.set_value(model.linproj_v_std.get_value())
        self.linproj_s_std.set_value(model.linproj_s_std.get_value())
    ...
    def init_parameters(self):
        ...
        # determine truncation bounds for s
        self.alpha_std = 1./T.sqrt(self.alpha_prec)
        self.s_high = self.mu + self.alpha_std * self.truncation_bound['s'] if self.flags['truncate_s'] else None
        self.s_low  = self.mu - self.alpha_std * self.truncation_bound['s'] if self.flags['truncate_s'] else None
        # determine truncation bounds for t
        self.beta_std = 1./T.sqrt(self.beta_prec)
        self.t_high = self.nu + self.beta_std * self.truncation_bound['t'] if self.flags['truncate_t'] else None
        self.t_low  = self.nu - self.beta_std * self.truncation_bound['t'] if self.flags['truncate_t'] else None
    ...
    def init_chains(self):
        ...
        self.pos_t1_mu = sharedX(numpy.zeros((self.batch_size, self.n_g)), name='pos_t1_mu')
        self.pos_t1_mean = sharedX(numpy.zeros((self.batch_size, self.n_g)), name='pos_t1_mean')
        self.pos_t1_var = sharedX(numpy.zeros((self.batch_size, self.n_g)), name='pos_t1_var')
    ...
    def do_theano(self):
        ...
        lcost = self.ml_cost(
                        pos_g = self.pos_g,
                        pos_t1_mean = self.pos_t1_mean,
                        pos_t1_var = self.pos_t1_var,
                        pos_h = self.pos_h,
                        pos_s1_mean = self.pos_s1_mean,
                        pos_s1_var = self.pos_s1_var,
                        pos_s0_mean = self.pos_s0_mean,
    ...
    def get_constraint_updates(self):
        ...
        if self.flags['standardize_s']:
            ...
            #updates[self.alpha] = new_gamma_s / self.gamma_s * self.alpha
    ...
    def energy(self, g_sample, t_sample, h_sample, s_sample, v_sample):
        from_h = self.from_h(h_sample)
        from_s = self.from_s(s_sample - self.mu)
        from_v = self.from_v(v_sample)

        energy  = 0.
        # v-squared term
        energy += T.sum(0.5 * self.lambd_prec * v_sample**2, axis=1)
        # v-(h,s) interaction term and gaussian terms for s
        ...
        # s-(g,t) interaction term and gaussian terms for t
        energy -= T.sum(from_s * t_sample * g_sample, axis=1)
        energy += T.sum(0.5 * t_sample**2, axis=1)
        energy -= T.sum(self.beta_prec * self.nu * t_sample, axis=1)
        # bias terms
    ...
    def lbound_plus(self, g, t1_mean, t1_var, h, s1_mean, s1_var, s0_mean, s0_var, v,
        ...
        tt = g * (t1_mean**2 + t1_var)  + (1-g) * (0 + t1_var)
        
        from_v = self.from_v(v)
        from_s = self.from_s(s - self.mu)

        lq  = 0.
        lq -= T.sum(0.5 * self.lambd_prec * v**2, axis=1)
        lq -= T.sum(0.5 * self.alpha_prec * ss, axis=1)
        lq -= T.sum(0.5 * self.beta_prec * tt, axis=1)

        lq += T.sum(from_v * s1_mean * self.from_h(h), axis=1)
        lq += T.sum(self.alpha_prec * self.mu * s, axis=1)

        lq += T.sum(from_s * t1_mean * g, axis=1)
        lq += T.sum(self.beta_prec * self.nu * t, axis=1)

        lq += T.dot(g, self.gbias)
        lq += T.dot(h, self.hbias)
    ...
    def entropy_term(self, g, t1_mu, h, s1_mu, s0_mu):
        ...
        rval += T.sum(   h  * gauss_ent(s1_mu, self.alpha_std, self.s_low, self.s_high), axis=1)
        rval += T.sum((1-h) * gauss_ent(s0_mu, self.alpha_std, self.s_low, self.s_high), axis=1)
        # entropy from q(g,t)
        rval += T.sum(-T.xlogx.xlogx(g) - T.xlogx.xlogx(1-g), axis=1)
        rval += T.sum(   g  * gauss_ent(t1_mu, self.beta_std, self.t_low, self.t_high), axis=1)

        cte = [g, t1_mu, h, s1_mu, s0_mu]
        return rval, cte

    def lbound(self,
               g, t1_mu, t1_mean, t1_var,
               h, s1_mu, s1_mean, s1_var, s0_mu, s0_mean, s0_var,
               v, s_stats_const=True, t_stats_const=True):

        rval1, cte1 = self.lbound_plus(g, t1_mean, t1_var,
                h, s1_mean, s1_var, s0_mean, s0_var,
                v, s_stats_const, t_stats_const)
    ...
    def from_g(self, g_sample, center=True):
        if center and self.flags['center_g']:
            return g_sample - self.cg
        else:
            return g_sample

    def center_g(self, g_sample):
        return g_sample - self.cg if self.flags['center_g'] else g_sample
    
    def center_h(self, h_sample):
        return h_sample - self.ch if self.flags['center_h'] else h_sample

    def from_gt(self, g_sample, t_sample):
        return T.dot(t_sample * g_sample, self._Wg.T)
    ...
    def g_given_s(self, s_sample, eps=1e-6):
        ...
        if self.flags['truncate_t']:
            ...
            log_Z_t1, _none = log_trunc_gauss_Z(t_mu_g1, self.beta_std, self.t_low, self.t_high)
            log_Z_t0, _none = log_trunc_gauss_Z(t_mu_g0, self.beta_std, self.t_low, self.t_high)
    ...
    def t_mu_given_gs(self, g_sample, s_sample):
        from_s = self.from_s(s_sample - self.mu)
        t_mean = self.nu + 1./self.beta_prec * from_s * g_sample
        return t_mean
    ...
    def sample_t_given_gs(self, g_sample, s_sample, rng=None, size=None):
        ...
        """
        t_mu = self.t_mu_given_gs(g_sample, s_sample)
    ...
    def h_given_gtv(self, g_sample, t_sample, v_sample, eps=1e-6):
        ...
        h_mean_s = from_v * (self.mu + from_gt)
        ...
        if self.flags['truncate_s']:
            ...
            log_Z_s1, _none = log_trunc_gauss_Z(s_mu_h1, self.alpha_std, self.s_low, self.s_high)
            log_Z_s0, _none = log_trunc_gauss_Z(s_mu_h0, self.alpha_std, self.s_low, self.s_high)
    ...
    def s_mu_given_gthv(self, g_sample, t_sample, h_sample, v_sample):
        ...
        from_v = self.from_v(v_sample)
        s_mu = self.mu
        s_mu += 1./self.alpha_prec * from_v * from_h
        s_mu += from_gt
        return s_mu
    ...
    def v_given_hs(self, h_sample, s_sample):
        from_h = self.from_h(h_sample)
        v_mean = T.dot(s_sample * from_h, self._Wv.T)
        return v_mean
    ...
    def pos_phase(self, v, init_state, n_steps=1, eps=1e-3):
        ...
        """
        def pos_mf_iteration(g, t1_mu, h, s1_mu, s0_mu, v, pos_counter):
            """
            Mean-field computes the following statistics:
            E[h], E[s|h=1], E[s|h=0], E[s], E[g], E[t|g=1], E[t|g=0]=0.
            """
            s_low  = -self.truncation_bound['s'] if self.flags['truncate_s'] else None
            s_high = +self.truncation_bound['s'] if self.flags['truncate_s'] else None
            t_low  = -self.truncation_bound['t'] if self.flags['truncate_t'] else None
            t_high = +self.truncation_bound['t'] if self.flags['truncate_t'] else None

            dummy = T.constant(0, dtype=floatX)

            def dbg_track_lbound(g, t1_mu, h, s1_mu, s0_mu, v, i=0):
                (s1_mean, s1_var) = self.trunc_stats(s1_mu, T.ones((self.n_s,)), s_low, s_high)
                (s0_mean, s0_var) = self.trunc_stats(s0_mu, T.ones((self.n_s,)), s_low, s_high)
                (t1_mean, t1_var) = self.trunc_stats(t1_mu, T.ones((self.n_g,)), t_low, t_high)
                lbound, _crap = self.lbound(g, t1_mu, t1_mean, t1_var,
                        h, s1_mu, s1_mean, s1_var, s0_mu, s0_mean, s0_var, v)
                return Print('lbound%i'%i)(T.sum(lbound))
            ...
            new_s1_mu = self.s_mu_given_gthv(g, t1_mean, T.ones((v.shape[0],  self.n_h)), v)
            if self.flags['debug_inference']:
                dummy += dbg_track_lbound(g, t1_mu, new_h, new_s1_mu, s0_mu, v, i=2)

            # (3) p(s|h=0) := N(new_s0_mu, 1./self.alpha_prec)
            new_s0_mu = self.s_mu_given_gthv(g, t1_mean, T.zeros((v.shape[0], self.n_h)), v)
            if self.flags['debug_inference']:
                dummy += dbg_track_lbound(g, t1_mu, new_h, new_s1_mu, new_s0_mu, v, i=3)

            # IMPORTANT: for truncation gaussian, E[s1] != mu 
            (new_s1_mean, new_s1_var) = self.trunc_stats(new_s1_mu, T.ones((self.n_s,)), s_low, s_high)
            (new_s0_mean, new_s0_var) = self.trunc_stats(new_s0_mu, T.ones((self.n_s,)), s_low, s_high)

            # new_s := E[s|h=1] p(h=1) + E[s|h=0] p(h=0)
            new_s = new_s1_mean * self.from_h(new_h) + new_s0_mean * (1-self.from_h(new_h))
            ...
            # (5) p(t|g=1) := N(new_t1_mu, 1./self.beta_prec)
            new_t1_mu = self.t_mu_given_gs(T.ones((v.shape[0], self.n_g)), new_s)
            if self.flags['debug_inference']:
                dummy += dbg_track_lbound(new_g, new_t1_mu, new_h, new_s1_mu, new_s0_mu, v, i=4)

            # IMPORTANT: for truncation gaussian, E[t1] != mu 
            (new_t1_mean, new_t1_var) = self.trunc_stats(new_t1_mu, T.ones((self.n_g,)), t_low, t_high)
            ################# DONE ITERATION #############

            if self.flags['debug_inference']:
                # ensure that derivative goes to 0 as bound gets tighter
                dlbound_args = [new_g, new_t1_mu, new_t1_mean, new_t1_var,
                ...
                stop = T.maximum(T.max(new_g - g),
                         T.maximum(T.max(new_h - h),
                            T.maximum(T.max(new_t1_mu - t1_mu),
        ...
        states = [T.unbroadcast(T.shape_padleft(init_state['g'])),
                  T.unbroadcast(T.shape_padleft(init_state['t1_mu'])),
                  {'steps': 1},  # t1_mean
                  {'steps': 1},  # t1_var
    ...
    def pos_phase_updates(self, v, init_state=None, n_steps=1):
        ...
        if init_state is None:
            ...
            init_state = OrderedDict()
            init_state['g'] = T.ones((v.shape[0], self.n_g)) * T.nnet.sigmoid(self.gbias)
            init_state['t1_mu'] = T.zeros((v.shape[0], self.n_g))
            init_state['h'] = T.ones((v.shape[0], self.n_h)) * T.nnet.sigmoid(self.hbias)
        ...
        pos_updates[self.pos_t1_mu]   = rval[1]
        pos_updates[self.pos_t1_mean] = rval[2]
        pos_updates[self.pos_t1_var]  = rval[3]
        pos_updates[self.pos_h]       = rval[4]
        pos_updates[self.pos_s1_mu]   = rval[5]
        pos_updates[self.pos_s1_mean] = rval[6]
        pos_updates[self.pos_s1_var]  = rval[7]
        pos_updates[self.pos_s0_mu]   = rval[8]
        pos_updates[self.pos_s0_mean] = rval[9]
        pos_updates[self.pos_s0_var]  = rval[10]
        # v:=rval[11] is not used.
        pos_updates[self.pos_counter] = rval[12]
        pos_updates[self.lbound_temp] = rval[13]

        # TODO: move to statistics_updates
        pos_g, pos_h = rval[0], rval[4]
        if self.flags['pos_phase_cg']:
            pos_updates[self.cg] = T.cast(0.99 * self.cg + 0.01 * pos_g.mean(axis=0), floatX)
        if self.flags['pos_phase_ch']:
            pos_updates[self.ch] = T.cast(0.99 * self.ch + 0.01 * pos_h.mean(axis=0), floatX)
    ...
    def ml_cost(self, pos_g, pos_t1_mean, pos_t1_var,
                      pos_h, pos_s1_mean, pos_s1_var, pos_s0_mean, pos_s0_var,
                      pos_v, neg_g, neg_t, neg_h, neg_s, neg_v):
        """
        ...
        # neg_cost := -log Z
        pos_cost, pos_cte = self.lbound_plus(
                pos_g, pos_t1_mean, pos_t1_var,
                pos_h, pos_s1_mean, pos_s1_var, pos_s0_mean, pos_s0_var,
                pos_v)
    ...
    def monitor_gauss(self, w, name=None):
        ...
        rval[name + '.max']  = w.max()
        rval[name + '.std_min']  = w.std(axis=0).min()
        rval[name + '.std_mean'] = w.std(axis=0).mean()
        rval[name + '.std_max']  = w.std(axis=0).max()
        return rval
    ...
    def get_monitoring_channels(self, x, y=None):
        ...
        chans.update(self.monitor_gauss(self.pos_t1_mu))
        #chans.update(self.monitor_gauss(self.pos_t1_mean))
        #chans.update(self.monitor_gauss(self.pos_t1_var))
    ...
    def dlbound_dg(self, g, t1_mu, t1_mean, t1_var,
                   h, s1_mu, s1_mean, s1_var, s0_mu, s0_mean, s0_var, v):

        lbound, cte = self.lbound(g, t1_mu, t1_mean, t1_var,
                h, s1_mu, s1_mean, s1_var, s0_mu, s0_mean, s0_var,
                v, t_stats_const=False)
        dlbound_dg = T.grad(T.sum(lbound), [g], consider_constant=cte)[0]
        return  g * (1-g) * dlbound_dg

    def dlbound_dh(self, g, t1_mu, t1_mean, t1_var,
                   h, s1_mu, s1_mean, s1_var, s0_mu, s0_mean, s0_var, v):

        lbound, cte = self.lbound(g, t1_mu, t1_mean, t1_var,
                h, s1_mu, s1_mean, s1_var, s0_mu, s0_mean, s0_var,
                v, s_stats_const=False)
