...
class BilinearSpikeSlabRBM(Model, Block):
    ...
    def init_parameters(self):
        ...
        self._alpha = -T.log(self.alpha)
        #self._alpha = self.alpha / self.gamma_s
    ...
    def energy(self, g_sample, t_sample, h_sample, s_sample, v_sample):
        ...
        energy -= T.sum(from_v * self._mu * self.from_h(ch_sample), axis=1)
        energy -= T.sum(from_v * self._alpha * s_sample * self.from_h(h_sample), axis=1)
        energy += 0.5 * T.sum(s_sample**2, axis=1)
        energy -= T.sum(from_s * self._nu * cg_sample, axis=1)
        energy -= T.sum(from_s * self.beta * t_sample * g_sample, axis=1)
        energy += 0.5 * T.sum(t_sample**2, axis=1)
    ...
    def lbound_plus(self, g, t1_mean, t1_var, h, s1_mean, s1_var, s0_mean, s0_var, v,
        ...
        lq -= 0.5 * T.sum(ss, axis=1)
        lq -= 0.5 * T.sum(tt, axis=1)

        lq += T.sum(from_v * self._mu * self.from_h(ch), axis=1)
        lq += T.sum(from_v * self._alpha * s1_mean * from_h, axis=1)
        lq += T.sum(from_s * self._nu * cg, axis=1)
        lq += T.sum(from_s * self.beta * t1_mean * g, axis=1)
    ...
    def entropy_term(self, g, t1_mu, h, s1_mu, s0_mu):
        ...
        rval += T.sum(h * gauss_ent(s1_mu, 1.0, a, b), axis=1)
        rval += T.sum((1-h) * gauss_ent(s0_mu, 1.0, a, b), axis=1)
        # entropy from q(g,t)
    ...
    def from_s(self, s_sample):
        return T.dot(s_sample, self._Wg)
    ...
    def from_gt(self, g_sample, t_sample):
        temp  = self._nu * self.center_g(g_sample)
        temp += self.beta * t_sample * g_sample
    ...
    def g_given_s(self, s_sample, eps=1e-6):
        ...
        if self.flags['truncate_t']:
            ...
            log_Z_t1, _none = log_trunc_gauss_Z(t_mu_g1, 1.0, -b, b)
            log_Z_t0, _none = log_trunc_gauss_Z(t_mu_g0, 1.0, -b, b)
    ...
    def t_mu_given_gs(self, g_sample, s_sample):
        from_s = self.from_s(s_sample)
        t_mean = from_s * self.beta * g_sample
        return t_mean
    ...
    def h_given_gtv(self, g_sample, t_sample, v_sample, eps=1e-6):
        ...
        temp = self._mu + self._alpha * self.from_gt(g_sample, t_sample)
        h_mean_s = from_v * temp
        h_mean_s += 0.5 * (self._alpha * from_v)**2
        ...
        if self.flags['truncate_s']:
            ...
            log_Z_s1, _none = log_trunc_gauss_Z(s_mu_h1, 1.0, -b, b)
            log_Z_s0, _none = log_trunc_gauss_Z(s_mu_h0, 1.0, -b, b)
    ...
    def s_mu_given_gthv(self, g_sample, t_sample, h_sample, v_sample):
        ...
        from_h = self.from_h(h_sample)
        from_v = self.from_v(v_sample)
        s_mu = from_v * self._alpha * from_h + from_gt
        return s_mu
    ...
    def v_given_hs(self, h_sample, s_sample):
        temp  = self._mu * self.from_h(self.center_h(h_sample))
        temp += self._alpha * s_sample * self.from_h(h_sample)
        v_mean =  T.dot(temp, self._Wv.T)
        return v_mean
